            ...
            if (!empty($_POST['data'])) {
                ...
                if (!empty($_POST['lasf_options_nonce']) && wp_verify_nonce($_POST['lasf_options_nonce'], 'lasf_options_nonce')) {
                    ...
                    wp_send_json_success(array(
                        ...
                        'notice' => $this->notice,
                        'errors' => $this->errors,
	                    'fragments' => apply_filters('lasf_setting_fragments_element_reload', [], $this->unique, $this)
                    ));
                }
        ...
        public function set_options() {
            if (wp_verify_nonce(lasf_get_var('lasf_options_nonce'), 'lasf_options_nonce')) {
                ...
                if (!empty($transient['lasf_import_data'])) {
                    ...
                    if(isset($request['header_layout'])){
                    	unset($request['header_layout']);
                    }
                ...
                $arr_key = apply_filters('lasf_setting_key_to_ignore', ['la_custom_css', 'footer_js', 'header_js', 'header_code', 'footer_code']);
                $arr_bk = [];
                if(!empty($arr_key) && is_array($arr_key)){
                    foreach ($arr_key as $item){
                        if(isset($request[$item])){
                            $arr_bk[$item] = $request[$item];
                        }
                    }
                }
                $request = wp_unslash($request);
                if(!empty($arr_bk)){
                    $request = array_merge($request, $arr_bk);
                }

                $request = apply_filters("lasf_{$this->unique}_save", $request, $this);
                do_action("lasf_{$this->unique}_save_before", $request, $this);
                do_action("lasf_theme_setting_save_before", $request, $this);
